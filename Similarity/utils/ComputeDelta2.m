function [delta, Kmin] = ComputeDelta2(epsilon,sysLTI,mu,sigma,Bset,D,varargin)
...
for i = 1:length(epsilon)
    ...
    for k = 1:length(lambda_v)
        ...
        if interfaceK
            ...
            LMI = [(r_sq>=0); (M2>=0); (M2b>=0)]; % alt: r_sq>=1e-6
        else
            LMI = [(r_sq>=0); (M2>=0)]; % alt: r_sq>=1e-6
        end
        ...
        for l = 1:size(Bset.V,1)
                ...
                M3 =    [lambda*Dinv,zeros(dim,1), Dinv*A'+Q'*B'+L'*Bw';
                        zeros(1,dim), (alpha^2-lambda)*eps^2, beta';
